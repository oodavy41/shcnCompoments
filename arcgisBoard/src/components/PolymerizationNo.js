...
export default class Polymerization extends Component {
  ...
  cluster(datas) {
    ...
    let drawCluster = (pointraw, icon, popup) => {
      var mark = {
        type: "picture-marker",
        url: icon || roadOcc,
        width: "10px",
    ...
    for (let i = 0; i < datas.length; i++) {
      if (!datas[i].hide) {
        ...
        this.pointsObj = this.pointsObj.concat(
          drawCluster(e.points, e.marks, e.popup)
        );
      }
    }
  }
      ...
      .then(
        ([
          ...
          var JiChang = new FeatureLayer({
            url:
              "http://map.cn.gov/OneMapServer/rest/services/JTSS_FCLASS/FeatureServer/6",
            renderer: jcRenderer,
          });

...
(function (win, doc) {
  const MapServerToken =
    process.env.NODE_ENV === "production"
      ...
      : "http://map.cn.gov/RemoteTokenServer?request=getToken&username=cnkwkf&password=cnkwkf12345&clientid=ref.http://localhost:7000/&expiration=500";
  ...
  win.getToken = function () {
    axios.get(MapServerToken).then((response) => {
      document.getElementById("txtToken").value = response.data;
      console.log(response);
    });
  };

  const ipHesheng = "bigdata.cn.gov";
  const portHs = 8080;
  ...
  win.conf = {
    ...
    hesheng: {
      ...
      alarmYesSum: `${HsDomain}/visdata/rest//dataset/changingyesterdaysum/result`,
      Alarm30Sum: `${HsDomain}/visdata/rest//dataset/changningsum/result`,
